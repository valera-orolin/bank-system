...
if ($min_amount < 0 || $max_amount < 0) {
    echo "<script>alert('Failed to update a deposit type. Minimal and maximal amounts must be positive numbers.'); window.location.href='/controller/deposit_types/index.php';</script>";
    die();
}

if ($min_amount >= $max_amount) {
    echo "<script>alert('Failed to update a deposit type. Maximal amount must be greater than minimal.'); window.location.href='/controller/deposit_types/index.php';</script>";
    die();
}
